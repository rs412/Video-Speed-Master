...
(() => {
  ...
  const DEFAULTS = {
    ...
    showOsd: true,
    bgPlay: false,
    dual: false, // 双开防暂停（实验）：由页面 localStorage __vsm_dual__ 在重载后控制
  ...
  function applyDualFlag() {
    try { localStorage.setItem(DUAL_KEY, cfg.dual ? '1' : '0'); } catch (_) {}
  }
  ...
  function save(patch) {
    ...
    if (cfg.scope === 'tab') {
      ...
      chrome.storage.sync.set({ vsmGlobal: {
        ...
        bgPlay: cfg.bgPlay, dual: cfg.dual, scope: cfg.scope, shortcuts: cfg.shortcuts,
      }});
      if (cfg.rememberSite) {
        chrome.storage.local.get(['vsmSites'], (l) => {
