...
(() => {
  ...
  /* 同样劫持 defaultPlaybackRate：部分播放器会写回默认速度来复位 */
  const dNative = Object.getOwnPropertyDescriptor(proto, 'defaultPlaybackRate');
  if (dNative && dNative.set && !dNative.set.__vsmPatched) {
    const dPatched = function (value) {
      const target = targets.get(this);
      if (state.enabled && state.force && target != null &&
          Math.abs(value - target) > EPS) {
        return; // 页面试图把默认速度改回，吞掉
      }
      dNative.set.call(this, value);
    };
    dPatched.__vsmPatched = true;
    Object.defineProperty(proto, 'defaultPlaybackRate', {
      configurable: true,
      enumerable: dNative.enumerable,
      get: dNative.get,
      set: dPatched,
    });
  }
  ...
  function scan(deep) {
    const found = deep ? deepCollect(document, [], 8) : collect(document, []);
  ...
  function hook(v) {
    ...
    ['ratechange', 'play', 'playing', 'loadedmetadata', 'durationchange',
     'emptied', 'loadstart', 'canplay', 'seeked', 'timeupdate'].forEach((ev) =>
